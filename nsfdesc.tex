...
\paragraph{\emph{Population Genomic Analyses:}} \jri{do we mention that we know GBS screws up hets badly, but that we know of methods that should work fine regardless?}
...
Finally, if phasing genotypes proves difficult given high levels of missing data or heterozygous error, we will make use of software designed to estimate admixture from genotype-likelihoods calculated on low-coverage sequence data \citep{skotte2013estimating}. 
...
Finally, this aim will also provide useful baseline information on patterns of genetic diversity in \zl and \zh, both taxa of conservation concern within Gautemala and of interest for novel root traits for breeding including root angle, adventitious root formation, and the formation of arenchyma \citep{omori2007qtl,mano2007breeding}.

dditional questions: how much diversity remains in lux populations? what is connectivity of these pops? are lux and/or huehue threatened by introgression from maize? important to Guatemala as it is unique diversity to that country
...
And while our previous work points to an important role of introgression in facilitating the rapid adaptation of maize to new environments\citep{Hufford2013}, the geographic scale of adaptive introgression is not well understood.  
We identified a number of genomic regions showing evidence of introgression from \zm into maize across the highlands, suggesting that some \zm alleles increased the fitness of maize across a wide geographic area.  
We also observed a number of regions showing evidence of introgression in only a subset of the sympatric populations, but ascertainment bias and the limited resolution of the SNPs available prevented further analysis of these loci.

Here, we propose to analyze a set of sympatric pairs of maize and teosinte populations in the highlands of Central Mexico (with \zm), the Pacific coast of Mexico (with \zp), and the lowlands of Guatemala (with \zl and \zh). In addition to the populations sampled in \ref{sss:adaptive_intro}, we will sample 3 pairs of \zm and \zp previously identified as sympatric with domesticated maize \citep{hufford2010genetic, Hufford2013}.  Maize and teosinte individuals from each population will be genotyped using GBS, and we will test for introgression and selection following methods proposed in \ref{sss:adaptive_intro}.  
These data will allow us to assess the geographic scale of adaptive introgression.
...
While we have prior evidence for adaptive introgression from \zm into maize, we do not know whether introgression from \zp, \zl, or \zh may be beneficial in any population.
If introgression does appear to be beneficial, we may expect to find that it has been limited to only those regions which are broadly adaptive across a wide geographic area, as continual local gene flow from sympatric populations likely includes numerous deleterious alleles at domestication-related loci \citep[c.f.][]{Hufford2013}.
Alternatively, because there is considerable environmental variation even within a geographic area such as the Central highlands \citep{hufford2012inferences,Pyhajarvi2013}, we may observe evidence for substantial adaptive introgression in localized pairs of populations, with little overlap among populations. 
