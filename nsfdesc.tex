...
By expanding our study of hybridization and introgression across these taxa we can compare evidence supporting ecotone and tension zone hypotheses and, should hybridization prove adaptive, assess how the genomic landscape of hybridization is shaped by natural selection. \jri{this reads a bit like: let's do another hybrid zone. i think the cool part here is not whether one more example is an ecotone or not (because in the end that's just one more example) but to start getting at questions that haven't been addressed like whether similar dynamics play out in replicate hybrid zones (maybe one's an ecotone and the other a tension zone? maybe the loci that are adaptive differe, etc.)}
...
\jri{to me, the questions of interest are:
Objective1: Are the regions that show evidence of resistence to or selection for introgression in a hybrid zone the same as those that correlate with fitness or putatively adaptive phenotypes?
How strongly do hybrid zone dynamics depend on environment?  Are they replicable across zones?
Objective2: does gene flow from local relatives allow a colonizing/invasive species to more readily adapt to new environments?
What is the geographical scale on which introgression and local adaptation interact?
Can a widespread congener act as a bridge for gene flow between smaller allopatric taxa?

All of those could be better phrase, but seem to be of broad interest.  maize as a bridge by itself seems ho-hum until you point out that it's relevant to any colonizer. parv/mex as a hybrid zone seems ho-hum until you point out we have replicate hybrid zones that differe in distance from parent, etc. and while inversions are thought to protect locally adaptive variation we have evidence that inversions are enriched for adaptive introgression.

Anyway, you get the idea.  I don't want to rewrite this whole part if you're not cool with it, but this doesn't read very exciting to a non-teosinte person.}
